...
## Если вместо панели видишь только `+` или `++++`

Это значит, что на публичном порту отвечает не HTML-файл панели, а другой
обработчик/health-check. Корень сайта `/` должен отдавать
`web/static/index.html`, а не plain text.

### Быстрая проверка на сервере

```bash
curl -i http://127.0.0.1:24608/ | head -20
```

Правильный ответ начинается так:
...
Если вместо этого приходит `+`, `++++`, `ok` или другой текст — ты запустил не
тот web server или root-route backend возвращает health-check.

### Самый простой запуск встроенного сервера панели

В репозитории есть `web/server.py`; он без дополнительных зависимостей отдаёт
панель и статику:

```bash
WEB_HOST=0.0.0.0 WEB_PORT=24608 python main.py
```

Или только проверить UI без бота:
...
```text
http://IP_СЕРВЕРА:24608/
```

Важно: встроенный static-server отдаёт UI. Для рабочих `/api/*` endpoints всё
равно нужен полный backend бота или proxy `/api/*` на него.
